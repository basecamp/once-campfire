...
@Component({
  ...
  template: `
    <section class="panel">
      <header>
        <h1>Campfire</h1>
        <p>Node.js + Fastify + MongoDB + Angular 21</p>
      </header>
...
export default class LoginComponent {
  ...
  async submit(): Promise<void> {
    ...
    try {
      await this.auth.login(emailAddress, password);
      await this.store.init();
      await this.router.navigateByUrl('/chat');
    } catch {
      this.error.set('Ошибка авторизации. Проверьте данные и попробуйте снова.');
    }
  }
}
