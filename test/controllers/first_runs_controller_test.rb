...
class FirstRunsControllerTest < ActionDispatch::IntegrationTest
  FIRST_RUN_TOKEN = "first-run-test-token-abcdefghijklmnopqrstuvwxyz"
  ...
  test "new is permitted when no other users exit" do
    ...
    assert_select "input[name='bootstrap_token'][required]"
  end
    ...
    assert_difference -> { Room.count }, 1 do
      assert_difference -> { User.count }, 1 do
        post first_run_url, params: {
          bootstrap_token: FIRST_RUN_TOKEN,
  ...
  test "session creation failure discards a staged first-run avatar" do
    ...
    assert_no_difference [
      ...
      assert_raises(ActiveRecord::RecordInvalid) do
        post first_run_url, params: { bootstrap_token: FIRST_RUN_TOKEN, user: {
  ...
  test "invalid setup tokens cannot create the administrator or stage an avatar" do
    ...
    assert_no_difference [
      ...
      post first_run_url, params: {
        bootstrap_token: "attacker-token",
        user: {
          name: "Attacker", email_address: "attacker@example.com", password: "password123",
          avatar: fixture_file_upload("moon.jpg", "image/jpeg")
        }
      }
    end

    assert_response :unprocessable_entity
    assert_select "[role='alert']", text: /setup token does not match/i
    assert_select "input[name='bootstrap_token']"
  end
end
