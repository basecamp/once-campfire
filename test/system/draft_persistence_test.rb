...
class DraftPersistenceTest < ApplicationSystemTestCase
  ...
  test "complete pending-submission persistence denial keeps text and warns against reload" do
    user = users(:jz)
    room = rooms(:designers)
    body = "Keep this without browser storage"
    reject_session_storage_writes
    sign_in user.email_address
    join_room room
    page.execute_script <<~JAVASCRIPT
      History.prototype.replaceState = () => { throw new DOMException("blocked", "SecurityError") };
    JAVASCRIPT
    return_empty_message_success
    ...
    assert_text "Message outcome is unknown."
    assert_text "reloading could make a safe retry impossible"
    assert_button "Retry on this page"
    assert_equal body,
      page.evaluate_script("document.querySelector('trix-editor').editor.getDocument().toString().trim()")
    assert_not Message.exists?(creator: user, client_message_id: find(".message--failed")["data-client-message-id"])
    assert_nil page.evaluate_script(
      "sessionStorage.getItem(#{"campfire-draft:#{user.id}:#{room.id}:pending".to_json})"
    )
    assert_equal "false", find("html", visible: :all)["data-draft-storage-available"]
  end

  test "storage denial preserves and reconciles a committed client id after reload" do
    user = users(:jz)
    room = rooms(:designers)
    body = "Committed before storage-denied reload"
    reject_session_storage_writes
    sign_in user.email_address
    join_room room
    ...
    failed = find(".message--failed", text: body, wait: 5)
    client_message_id = failed["data-client-message-id"]
    history_record = page.evaluate_script("history.state.campfirePendingSubmissions")
    assert_equal client_message_id, history_record.fetch("submissions").sole.fetch("clientMessageId")
    assert_equal 1, room.messages.where(creator: user, client_message_id:).count
    ...
    Message.insert_all!(45.times.map do |index|
      {
        ...
        client_message_id: "storage-denial-later-#{index}",
    ...
    assert_selector ".message[data-message-id][data-client-message-id='#{client_message_id}']", text: body, wait: 10
    assert_equal 1, room.messages.where(creator: user, client_message_id:).count
    Timeout.timeout(5) do
      sleep 0.05 while page.evaluate_script("Boolean(history.state.campfirePendingSubmissions)")
    end
    assert_not page.evaluate_script("Boolean(history.state.campfirePendingSubmissions)")
  end
  ...
  private
    ...
    def return_empty_message_success
      execute_script_in_page_realm <<~JAVASCRIPT
        const returnEmptyMessageSuccess = (event) => {
          const { fetchOptions, url } = event.detail;
          const composerPath = new URL(document.querySelector("#composer").action).pathname;
          if (fetchOptions.method === "POST" && url.pathname === composerPath) {
            document.removeEventListener("turbo:before-fetch-request", returnEmptyMessageSuccess);
            event.detail.fetchRequest = {
        ...
        document.addEventListener("turbo:before-fetch-request", returnEmptyMessageSuccess);
      JAVASCRIPT
    end
